...
## Extracting Route Endpoints

```{r}
# Get top 3 desire lines
desire_top = desire_lines |>
  slice_max(order_by = all, n = 3)

# Extract start and end points
fromPlace = st_sf(
  data.frame(id = desire_top$from, geometry = lwgeom::st_startpoint(desire_top))
)
toPlace = st_sf(
  data.frame(id = desire_top$to, geometry = lwgeom::st_endpoint(desire_top))
)
```
...
```{r}
#| echo: false
# Load pre-calculated data for demonstration
routes_drive = read_sf("https://github.com/ITSLeeds/TDS/releases/download/22/routes_drive.geojson")
```
