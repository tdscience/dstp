...
## Alternative: Simple Remote Server Deployment

For simpler deployments (e.g., on a VPS or home server), follow these steps:
...
docker run -d \
  --name otp \
  -p 8080:8080 \
  -v otp-data:/opt/otp/graph \
  itsleeds/tds:latest
```

### 3. Set Up Reverse Proxy

For HTTPS access, configure a reverse proxy (Nginx, Caddy, or cloudflare):
...
```bash
curl https://your-domain.com/otp/routers/default
```
...
| Component | Command/Resource |
|-----------|------------------|
| Image | `itsleeds/tds:latest` |
| Port | 8080 (internal), 80/443 (proxy) |
| Graph data | Bundled in TDS image |
| Documentation | https://docs.opentripplanner.org/ |
