...
tm_shape(route_walk_gmaps) + tm_lines(col = "blue", lwd = 3) +
  # otp route for comparison
  tm_shape(route_walk) + tm_lines(col = "red", lwd = 3)
# Or static: plot(route_walk_gmaps)

# With route_google
routes_walk_gmaps2 = stplanr::route(
  l = desire_top,
  route_fun = stplanr::route_google,
  key = Sys.getenv("GOOGLE"),
  mode = "walking"
)
qtm(routes_walk_gmaps2)  # should be similar to route_walk_gmaps
```
